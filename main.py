...
def main():
    ...
    tuxmodifier.modify("FFFFFF","beak_shadow_on_eyes")
        ...
        if "left_eye" in key:
            for zone in left_eye:
                if 'left_pupil' in zone:
                    tuxmodifier.modify("FFFFFF", zone)
            ...
            for zone in right_eye:
                if 'reflection' in zone:
                    ...
                    tuxmodifier.modify(configparser.hexformat('Net'), zone)
        elif "beak" in key:
            for zone in beak:
                tuxmodifier.modify(configparser.hexformat('IP'), zone)
        elif "head" in key:
            for zone in head:
                tuxmodifier.modify(configparser.hexformat('CPU'), zone)
        elif "body" in key:
            for zone in body:
                tuxmodifier.modify(configparser.hexformat('devices'), zone)
        elif "torso" in key:
            for zone in torso:
                tuxmodifier.modify(configparser.hexformat('Kernel'), zone)
        elif "left_palm" in key:
            for zone in left_palm:
                tuxmodifier.modify(configparser.hexformat('USB'), zone)
        elif "right_palm" in key:
            print("hexformat(support) : ", configparser.hexformat('support'))
            for zone in right_palm:
                tuxmodifier.modify(configparser.hexformat('support'), zone)
